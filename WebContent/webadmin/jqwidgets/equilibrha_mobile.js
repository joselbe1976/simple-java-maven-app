function prepareMobile(id, ToolBar, Title,buttonTitle,UrlbuttonBack) {
	var structure ="";
      ...
      if (ToolBar == 1)
       {
    	  ...
    	  if (buttonTitle!="") 
    		  {
    		  	 Title = Title + "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;";
    		     var toolbar = $('<div id="Toolbar" style="border-bottom: 1px solid #555; z-index: 9999999999; position: absolute; top:0; width: 100%; height: 35px;"><div id="ToolbarButton" style="float: left; margin: 2px; margin-left: 2px; padding: 3px 10px;"><div style="float: left;"></div><span style="float: left;">'+buttonTitle+'</span><div style="clear:both;"></div></div><center>'+Title+'</center></div>');
    		  }
    	  else
    		  {
    		  	 var toolbar = $('<div id="Toolbar" style="border-bottom: 1px solid #555; z-index: 9999999999; position: absolute; top:0; width: 100%; height: 35px;"><div style="float: left;"></div><div style="clear:both;"></div><center>'+Title+'</center></div>');    		  
    		  }
       }
        ...
        if (!hasFullScreenParam) {
            ...
            if (ToolBar == 1){
	            $("#Toolbar").addClass('jqx-widget-header jqx-widget-header-' + theme);
	            $("#Toolbar").addClass('jqx-listmenu-header jqx-listmenu-header-' + theme);
	            $("#Toolbar").css('padding', '0px');
	            $("#Toolbar").css('padding-top', '5px');
            }
	
	            if (theme == "windowsphone") {
	            	if (ToolBar == 1){$("#Toolbar").css('background', "#2A2A2B");}
	            	if (ToolBar == 1){$("#Toolbar").css('border-bottom-color', "#000");}
	                $(document.body).css('background', "#000");
	                $(document.body).css('color', "#fff");
	            }
	            else if (theme == "android") {
	            	if (ToolBar == 1){$("#Toolbar").css('background', "#000");}
	            	if (ToolBar == 1){$("#Toolbar").css('border-bottom-color', "#333");}
	                $(document.body).css('background', "#000");
	                $(document.body).css('color', "#fff");
	            }
	            else if (theme === "mobile") {
	            	if (ToolBar == 1){$("#Toolbar").css('background', "#f8f8f8");}
	            	if (ToolBar == 1){$("#Toolbar").css('border-bottom-color', "#b2b2b2");}
	                $(document.body).css('background', "fff");
	                $(document.body).css('color', "#000");
	            }
	            if (ToolBar == 1)
	            { 	
		            if (buttonTitle!="") {
			            $("#ToolbarButton").jqxButton({ theme: theme });
			            $("#ToolbarButton").css('border-radius', '10px');
			            $("#ToolbarButton").find('div:first').addClass('jqx-listmenu-backbutton-arrow' + ' jqx-listmenu-backbutton-arrow-' + theme);
			            
			            //Evento click del boton
			            $("#ToolbarButton").click(function () {
	                      window.open(UrlbuttonBack, '_self');
			            });
         	  }
            }
